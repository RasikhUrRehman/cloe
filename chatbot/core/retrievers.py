...
class KnowledgeBaseRetriever:
    ...
    def semantic_search(
        self, query: str, top_k: int = None, filters: Optional[Dict[str, str]] = None
    ) -> List[Dict[str, Any]]:
        """
    ...
    def hybrid_search(
        ...
        hybrid_results = sorted(
            semantic_results, key=lambda x: x["hybrid_score"], reverse=True
        )[:top_k]
...
def main():
    ...
    results = retriever.retrieve(query=query, method=RetrievalMethod.SEMANTIC, top_k=3)
