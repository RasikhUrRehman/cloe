...
class CleoRAGAgent:
    ...
    def process_message(self, user_message: str) -> List[str]:
        ...
        try:
            ...
            if self._is_simple_acknowledgment(user_message):
                return ["Great! Let me continue with the next question."]
            else:
                return ["I'm having trouble processing that. Could you try rephrasing your response?"]
    ...
    def _pre_process_state_update(self, user_message: str):
        ...
        if (self.session_state.current_stage == ConversationStage.ENGAGEMENT and 
            user_lower in ['ok', 'okay', 'yes', 'sure'] and
            self.session_state.engagement and 
            not self.session_state.engagement.consent_given):
    ...
    def _enhance_input_with_context(self, user_message: str) -> str:
        ...
        acknowledgment_triggers = [
            'yes', 'sure', 'okay', 'ok', 'definitely', 'absolutely',
            'i am', 'i have', 'i can', 'i do', 'my', 'full-time', 'part-time'
        ]
        ...
        should_encourage = any(trigger in user_lower for trigger in acknowledgment_triggers + question_triggers)
    ...
    def _split_multi_messages(self, response: str) -> List[str]:
        ...
        if separator in response:
            ...
            if len(messages) > 1:
                logger.info(f"Intelligently split response into {len(messages)} messages")
                return messages
            else:
    ...
    def _detect_natural_breaks(self, response: str) -> List[str]:
        ...
        for pattern in break_patterns:
            ...
            if match:
                ...
                if first_part and rest and len(first_part) < 150:  # Don't split very long first parts
        ...
        if len(sentences) >= 3:  # Only split if we have enough content
            ...
            for i, sentence in enumerate(sentences[:-1]):  # Don't split at the last sentence
                ...
                if 0.3 <= current_chars / total_chars <= 0.7:
                    ...
                    ack_words = ['great', 'perfect', 'excellent', 'fantastic', 'wonderful', 'awesome', 'good']
    ...
    def _update_state_from_conversation(self, user_message: str, agent_response: str):
        ...
        if self.session_state.current_stage == ConversationStage.ENGAGEMENT:
            ...
            if any(word in user_lower for word in ['yes', 'sure', 'okay', 'ok', 'proceed', 'continue', 'agree']):
        ...
        # Qualification stage updates
            ...
            # Age confirmation
            if any(phrase in user_lower for phrase in ['yes', 'i am 18', 'i\'m 18', '18 or older', 'over 18', 'above 18']):
                if not qual.age_confirmed and ('age' in agent_lower or '18' in agent_lower):
                    qual.age_confirmed = True
                    state_changed = True
                    logger.info("Age confirmed")
            
            # Work authorization
            if any(phrase in user_lower for phrase in ['authorized', 'citizen', 'yes', 'legally authorized', 'work permit']):
                if not qual.work_authorization and ('work' in agent_lower and ('authorized' in agent_lower or 'authorization' in agent_lower)):
                    qual.work_authorization = True
                    state_changed = True
                    logger.info("Work authorization confirmed")
            
            # Shift preference detection
            shift_keywords = ['morning', 'day', 'afternoon', 'evening', 'night', 'overnight']
            ...
            date_patterns = [
                ...
                r'\basap\b', r'\bimmediately\b', r'\bright away\b', r'\btomorrow\b'
            ]
            if not qual.availability_start:
                for pattern in date_patterns:
                    if re.search(pattern, user_lower):
                        qual.availability_start = user_message  # Store the full response for now
                        state_changed = True
                        logger.info("Availability start date captured")
                        break
            
            # Transportation
            if any(phrase in user_lower for phrase in ['yes', 'have transportation', 'can get there', 'own car', 'reliable transportation']):
        ...
        # Application stage updates
            ...
            if self._is_application_complete(app):
                if not app.stage_completed:
                    ...
                    state_changed = True
                    logger.info("Application completed - Fit score calculated - Moving to VERIFICATION stage")
        ...
        if state_changed:
            self.state_manager.save_session(self.session_state)
            logger.info(f"State updated and saved for session {self.session_state.session_id}")
    ...
    def _extract_information_proactively(self, user_message: str):
        ...
        age_patterns = ['18', 'nineteen', 'twenty', 'over 18', 'older than 18', 'above 18']
        if not qual.age_confirmed and any(pattern in user_lower for pattern in age_patterns):
            qual.age_confirmed = True
            state_changed = True
            logger.info("Proactively captured age confirmation")
        
        # Work authorization patterns
        work_auth_patterns = ['authorized to work', 'citizen', 'work permit', 'legally work', 'work visa']
        if not qual.work_authorization and any(pattern in user_lower for pattern in work_auth_patterns):
            qual.work_authorization = True
            state_changed = True
        ...
        if not qual.hours_preference:
            if any(phrase in user_lower for phrase in ['full-time', 'full time', 'fulltime']):
                qual.hours_preference = 'full-time'
                state_changed = True
                logger.info("Proactively captured hours preference: full-time")
            elif any(phrase in user_lower for phrase in ['part-time', 'part time', 'parttime']):
                qual.hours_preference = 'part-time'
                state_changed = True
                logger.info("Proactively captured hours preference: part-time")
        
        # Transportation
        if not qual.transportation and any(phrase in user_lower for phrase in ['have transportation', 'own car', 'reliable transport', 'can get there']):
            qual.transportation = True
            state_changed = True
        ...
        if not app.full_name:
            for pattern in name_patterns:
                ...
                if name_match:
                    ...
                    false_positives = ['interested', 'looking', 'good', 'ready', 'over', 'older', 'above', 'under', 'authorized', 'eighteen']
                    if (len(name) > 1 and 
                        not any(word in name.lower() for word in false_positives) and 
                        len(name.split()) >= 2):  # Require at least first and last name
                        app.full_name = name
                        state_changed = True
